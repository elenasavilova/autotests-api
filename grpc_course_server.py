...
class CourseServiceServicer(course_service_pb2_grpc.CourseServiceServicer):
    ...
    def GetCourse(self, request, context):
        ...
        print(f'Получен запрос к методу GetCourse, id курса: {request.course_id}')

        # Формируем и возвращаем ответное сообщение
        return course_service_pb2.GetCourseResponse(course_id=request.course_id, title="Автотесты API", description=
        "Будем изучать написание API автотестов")


# Функция для запуска gRPC-сервераpython -m grpc_course_server
def serve():
